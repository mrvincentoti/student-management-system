    ...
    <div class="table-responsive">
      <table class="table table-bordered table-hover">
        <thead>
          <tr>
            <th scope="col">#</th>
            <th scope="col">@lang('Student Code1')</th>
            <th scope="col">@lang('Student Name')</th>
            <!-- <th scope="col">@lang('Attendance')</th>
            @for($i=1;$i<=1;$i++) <th scope="col">@lang('Quiz') {{$i}}</th>
              @endfor
              @for($i=1;$i<=1;$i++) <th scope="col">@lang('Assignment') {{$i}}</th>
                @endfor -->
            @for($i=1;$i<=1;$i++) <th scope="col">@lang('CA')</th>
              @endfor
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><a href="{{url('user/'.$grade->student->id)}}">{{$grade->student->name}}</a></td>
            <!-- <td>{{$grade->attendance}}</td>
            @for($i=1;$i<=1;$i++) <td>{{$grade['quiz'.$i]}}</td>
              @endfor
              @for($i=1;$i<=1;$i++) <td>{{$grade['assignment'.$i]}}</td>
                @endfor -->
            @for($i=1;$i<=1;$i++) <td>{{$grade['ct'.$i]}}</td>
              @endfor
              @if($grade->course->final_exam_percent > 0)
              <td>{{$grade->written}}</td>
              <!--td>{{$grade->mcq}}</td-->
