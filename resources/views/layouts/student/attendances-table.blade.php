    ...
    if (count($attendances) > 0) {
      ...
      foreach ($attendances as $attendance) {
        if ($attendance->present == 1) {
          $events[] = ['title' => "Present", 'start' => $attendance->created_at, 'end' => $attendance->updated_at, 'color' => '#54ca68'];
        } else if ($attendance->present == 2) {
          $events[] = ['title' => "Escaped", 'start' => $attendance->created_at, 'end' => $attendance->updated_at, 'color' => '#ffc107'];
        } else {
          $events[] = ['title' => "Absent", 'start' => $attendance->created_at, 'end' => $attendance->updated_at, 'color' => '#fc544b'];
        }
      }
    } ?>
    <script>
      document.addEventListener('DOMContentLoaded', function() {
        ...
        var attEvents = <?php if (sizeof($events) > 0) {
